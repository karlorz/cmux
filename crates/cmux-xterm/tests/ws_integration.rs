...
async fn ws_echo_cat() {
    ...
    let resp = tokio::time::timeout(Duration::from_secs(10), async {
        client
            ...
            .send()
            .await
    }).await.expect("create tab timed out").unwrap();
    ...
    }).await.expect("ws connect timed out").unwrap();
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            if let Some(Ok(msg)) = ws.next().await {
                if let Message::Text(t) = msg { if t.contains("hello world") { break; } }
            }
        }
    }).await.expect("did not receive echo in time");
    ...
    // Cleanup
    let resp = client.delete(format!("{}/api/tabs/{}", base, id)).send().await.unwrap();
