...
import { Loader2, ChevronDown, ChevronRight, Check, Circle, Clock, X, AlertCircle, ExternalLink } from "lucide-react";
...
type CheckRunDoc = Doc<"githubCheckRuns">;
type DeploymentDoc = Doc<"githubDeployments">;
type CommitStatusDoc = Doc<"githubCommitStatuses">;

type CombinedWorkflowRunId =
  | WorkflowRunDoc["_id"]
  | CheckRunDoc["_id"]
  | DeploymentDoc["_id"]
  | CommitStatusDoc["_id"];

export interface CombinedWorkflowRun {
  _id: CombinedWorkflowRunId;
...
export function useCombinedWorkflowData({ teamSlugOrId, repoFullName, prNumber, headSha }: WorkflowRunsParams): {
  allRuns: CombinedWorkflowRun[];
  isLoading: boolean;
} {
  if (!repoFullName || prNumber <= 0) {
    return { allRuns: [] as CombinedWorkflowRun[], isLoading: false };
  }
  ...
  const allRuns = useMemo<CombinedWorkflowRun[]>(() => {
    const runs: CombinedWorkflowRun[] = [];

    const workflowRunDocs = (workflowRuns ?? []) as WorkflowRunDoc[];
    for (const run of workflowRunDocs) {
      runs.push({
        ...run,
        type: "workflow",
    ...
    const checkRunDocs = (checkRuns ?? []) as CheckRunDoc[];
    for (const run of checkRunDocs) {
      const url =
        run.htmlUrl ||
    ...
    const deploymentDocs = (deployments ?? []) as DeploymentDoc[];
    for (const dep of deploymentDocs) {
    ...
    const statusDocs = (commitStatuses ?? []) as CommitStatusDoc[];
...
export function WorkflowRuns({ allRuns, isLoading }: { allRuns: CombinedWorkflowRun[]; isLoading: boolean }) {
...
export function WorkflowRunsSection({
  ...
  const getStatusDescription = (run: CombinedWorkflowRun) => {
    const parts: string[] = [];
