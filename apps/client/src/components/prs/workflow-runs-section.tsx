...
export function WorkflowRuns({ allRuns, isLoading }: WorkflowRunsProps) {
...
export interface WorkflowRunsSectionProps {
  allRuns: CombinedRun[];
  isLoading: boolean;
  isExpanded: boolean;
  onToggle: () => void;
  ...
  return (
    <div className={containerClassName}>
      <button
        type="button"
        onClick={onToggle}
        className="w-full flex items-center justify-between pl-3 pr-2.5 py-1.5 border-y border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900 hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors"
      >
      ...
      {isExpanded && (
        <div className="divide-y divide-neutral-200 dark:divide-neutral-800 border-b border-neutral-200 dark:border-neutral-800">
          {sortedRuns.map((run, index) => {
            const appLabel =
              run.type === "check" && typeof run.appSlug === "string" && run.appSlug
                ? `[${run.appSlug}]`
                : run.type === "check" && typeof run.appName === "string" && run.appName
                  ? `[${run.appName}]`
                  : run.type === "deployment"
                    ? "[deployment]"
                    : run.type === "status"
            ...
            return (
              <a
                key={`${run.type}-${run._id ?? run.timestamp ?? index}`}
                href={run.url || "#"}
                target="_blank"
                rel="noreferrer"
