...
export type ScreenshotStatus = "completed" | "failed" | "skipped";
...
export interface ScreenshotGalleryProps {
  ...
  compact?: boolean;
  showHeader?: boolean;
  onImageClick?: (image: ScreenshotImage, set: ScreenshotSet) => void;
}
...
export function ScreenshotGallery({
  ...
  const handleImageClick = useCallback(
    (image: ScreenshotImage, set: ScreenshotSet, stableKey: string) => {
      if (onImageClick) {
        onImageClick(image, set);
      } else {
        setActiveImageKey(stableKey);
      }
    },
    [onImageClick],
  );
  ...
  const sectionClassName = cn(
    compact
      ...
      : "border-b border-neutral-200 bg-neutral-50/60 dark:border-neutral-800 dark:bg-neutral-950/40",
    className,
  );
  ...
  return (
    <section className={sectionClassName}>
      ...
      <div className={cn("space-y-4", compact ? "px-3 pb-3" : "px-3.5 pb-4")}>
        {currentEntry ? (
          <Dialog.Root
        ...
        {compact ? (
          <div className="flex gap-2 overflow-x-auto pb-1">
            {flattenedImages.map((entry) => {
              ...
              const displayName = entry.image.fileName ?? "Screenshot";
              const humanIndex = entry.globalIndex + 1;

              return (
                <button
                  key={stableKey}
                  type="button"
                  onClick={() => handleImageClick(entry.image, entry.set, stableKey)}
                  className="group relative flex h-32 w-48 flex-shrink-0 flex-col overflow-hidden rounded-lg border border-neutral-200 bg-neutral-50 text-left transition-colors hover:border-neutral-400 dark:border-neutral-700 dark:bg-neutral-900/70 dark:hover:border-neutral-500"
                  aria-label={`View ${displayName}`}
                >
                  <img
                    src={entry.image.url ?? undefined}
                  ...
                  <span className="pointer-events-none absolute bottom-1 left-1 rounded-full bg-neutral-950/80 px-1.5 py-0.5 text-[10px] font-semibold text-white shadow-sm dark:bg-neutral-900/90">
                    {humanIndex}
                  </span>
                </button>
              );
                    ...
                    {set.images.map((image, indexInSet) => {
                      ...
                      return (
                        <button
                          key={stableKey}
                          type="button"
                          onClick={() => handleImageClick(image, set, stableKey)}
                          className={cn(
