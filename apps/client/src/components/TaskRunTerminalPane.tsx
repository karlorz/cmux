...
export function TaskRunTerminalPane({ workspaceUrl }: TaskRunTerminalPaneProps) {
  ...
  // Set active terminal to first available
  if (hasTerminalBackend && terminalIds.length > 0 && !activeTerminalId) {
    setActiveTerminalId(terminalIds[0]);
  }

  // Update connection states when tabs change
  if (hasTerminalBackend && terminalIds.length > 0) {
    const newStates: Record<string, TerminalConnectionState> = {};
    for (const id of terminalIds) {
      newStates[id] = connectionStates[id] ?? "connecting";
    }
    const statesChanged =
      Object.keys(connectionStates).length !== Object.keys(newStates).length ||
      terminalIds.some(id => connectionStates[id] !== newStates[id]);
    if (statesChanged) {
    ...
    if (!xtermBaseUrl) {
      return (
        <div className="flex h-full flex-col items-center justify-center gap-2 text-sm text-neutral-500 dark:text-neutral-400">
          ...
          <span>Waiting for workspace to expose the terminal backend...</span>
        </div>
      );
    }
                  ...
                  <div key={id} className="relative pr-2">
                    ...
                    <button
                      ...
                      className={clsx(
                        ...
                        isActive
                          ? "text-white hover:text-white hover:bg-neutral-600 dark:text-white dark:hover:text-white dark:hover:bg-neutral-600"
          ...
          <div className="flex flex-col items-end gap-1 py-2">
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1 rounded-md border border-neutral-200 bg-neutral-700 px-2 py-1 text-xs font-medium text-white transition hover:border-neutral-300 hover:bg-neutral-600 disabled:cursor-not-allowed disabled:opacity-60 dark:border-neutral-700 dark:bg-neutral-700 dark:text-white dark:hover:border-neutral-600 dark:hover:bg-neutral-600"
              >
                {isCreatingTerminal ? (
                  "Creating…"
                ) : (
  ...
  return (
    <div className="flex flex-col grow h-full bg-black dark:bg-black">
      <div className="flex flex-col grow min-h-0 h-full">
        {renderTerminalArea()}
      </div>
    </div>
