...
export function WorkspaceCreationButtons({
  ...
  const handleCreateCloudRepoWorkspace = useCallback(async () => {
    ...
    try {
      ...
      const taskId = await createTask({
        teamSlugOrId,
        text: `Cloud Repo Workspace: ${projectFullName}`,
        projectFullName,
        baseBranch: "main",
        environmentId: undefined,
        isCloudRepoWorkspace: true,
      });
      ...
      await new Promise<void>((resolve) => {
        socket.emit(
          ...
          async (response: CreateCloudRepoWorkspaceResponse) => {
            if (response.success) {
              toast.success("Cloud repo workspace created successfully");
            } else {
              toast.error(
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            ...
            <span>Create Cloud Env Workspace</span>
          </button>
        </TooltipTrigger>
        <TooltipContent>
          {!selectedProject.length
            ...
            : !isEnvSelected
              ...
              : "Create cloud workspace from selected environment"}
        </TooltipContent>
      </Tooltip>
    </div>
  );
