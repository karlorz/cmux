...
export interface UseTaskComposerResult {
  ...
  handleProjectSearchPaste: (input: string) => Promise<boolean>;
  ...
  canSubmit: boolean;
  startTask: (editorApi?: EditorApi | null) => Promise<boolean>;
...
export function useTaskComposer({
  ...
  const reposByOrgQuery = useReactQuery({
  ...
  const environmentsQuery = useReactQuery(
  ...
  const persistAgentSelection = useCallback((agents: string[]) => {
    try {
      const normalizedAgents = filterKnownAgents(agents);
      const isDefaultSelection =
        normalizedAgents.length === DEFAULT_AGENT_SELECTION.length &&
        normalizedAgents.every(
          (agent, index) => agent === DEFAULT_AGENT_SELECTION[index]
        );

      if (normalizedAgents.length === 0 || isDefaultSelection) {
        localStorage.removeItem("selectedAgents");
      } else {
        localStorage.setItem(
          "selectedAgents",
          JSON.stringify(normalizedAgents)
        );
      }
    } catch (error) {
  ...
  const startTask = useCallback(
    async (editorApi?: EditorApi | null) => {
      ...
      try {
        ...
        const textContent = content?.text || taskDescription;
