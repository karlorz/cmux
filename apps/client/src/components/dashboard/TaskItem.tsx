...
export const TaskItem = memo(function TaskItem({
  ...
  const {
    ...
    renameInputRef,
    renameValue,
  } = useTaskRename({ task, teamSlugOrId });
  ...
  const renameInputElement = (
    <input
      ...
      spellCheck={false}
      className={clsx(
        "w-full bg-transparent text-[14px] font-medium leading-[20px] text-neutral-900 caret-neutral-600",
        "px-0 py-0 border-none outline-none focus-visible:outline-none focus-visible:ring-0 placeholder:text-neutral-400",
  ...
  const taskTitleElement = isRenaming ? (
    renameInputElement
  ) : (
    <span className={taskTitleClassName}>{task.text}</span>
  );
