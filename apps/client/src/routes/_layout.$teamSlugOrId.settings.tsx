...
import { API_KEY_TO_BASE_URL } from "@cmux/shared/apiKeys";
...
function SettingsComponent() {
  ...
  // Base URL settings state
  const [showBaseUrls, setShowBaseUrls] = useState(false);
  const [baseUrlValues, setBaseUrlValues] = useState<Record<string, string>>({});
  const [originalBaseUrlValues, setOriginalBaseUrlValues] = useState<Record<string, string>>({});
  const [bypassAnthropicProxy, setBypassAnthropicProxy] = useState(false);
  const [originalBypassAnthropicProxy, setOriginalBypassAnthropicProxy] = useState(false);
  ...
  useEffect(() => {
    if (existingKeys) {
      ...
      existingKeys.forEach((key: Doc<"apiKeys">) => {
        // Check if this is a base URL key
        if (key.envVar.endsWith("_BASE_URL")) {
          urlValues[key.envVar] = key.value;
        } else {
          apiValues[key.envVar] = key.value;
        }
      });

      setApiKeyValues(apiValues);
      setOriginalApiKeyValues(apiValues);
      setBaseUrlValues(urlValues);
      setOriginalBaseUrlValues(urlValues);
    }
  }, [existingKeys]);
  ...
  useEffect(() => {
    ...
    const nextBypassAnthropicProxy = workspaceSettings?.bypassAnthropicProxy ?? false;
  ...
  const hasChanges = () => {
    ...
    const baseUrlConfigs = Object.values(API_KEY_TO_BASE_URL);
    const baseUrlEnvVars = baseUrlConfigs.map(config => config.envVar);
    const baseUrlsChanged = baseUrlEnvVars.some((envVar) => {
      const currentValue = baseUrlValues[envVar] || "";
      const originalValue = originalBaseUrlValues[envVar] || "";
      return currentValue !== originalValue;
    });

    // Check bypass proxy toggle changes
    const bypassProxyChanged = bypassAnthropicProxy !== originalBypassAnthropicProxy;
  ...
  const saveApiKeys = async () => {
    ...
    try {
      ...
      // Save worktree path / auto PR / bypass proxy / heatmap settings if changed
      ...
      // Save base URLs (stored in the same apiKeys table)
      for (const baseUrlConfig of Object.values(API_KEY_TO_BASE_URL)) {
        const value = baseUrlValues[baseUrlConfig.envVar] || "";
        const originalValue = originalBaseUrlValues[baseUrlConfig.envVar] || "";

        // Only save if the value has changed
        if (value !== originalValue) {
          if (value.trim()) {
            // Save or update the base URL
            await saveApiKeyMutation.mutateAsync({
              envVar: baseUrlConfig.envVar,
              value: value.trim(),
              displayName: baseUrlConfig.displayName,
              description: baseUrlConfig.description,
            });
            savedCount++;
          } else if (originalValue) {
            // Delete the base URL if it was cleared
            await convex.mutation(api.apiKeys.remove, {
              teamSlugOrId,
              envVar: baseUrlConfig.envVar,
            });
      ...
      setOriginalBaseUrlValues(baseUrlValues);
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="md:w-[min(100%,480px)] md:flex-shrink-0 self-start">
                              ...
                              {/* Base URL input - shown when "Show base URLs" is toggled */}
                              {showBaseUrls && API_KEY_TO_BASE_URL[key.envVar] && (
                                <div className="mt-3 pt-3 border-t border-neutral-200 dark:border-neutral-800">
                                  <label
                                    htmlFor={API_KEY_TO_BASE_URL[key.envVar].envVar}
                                    className="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1"
                                  >
                                    {API_KEY_TO_BASE_URL[key.envVar].displayName}
                                  </label>
                                  <p className="text-xs text-neutral-500 dark:text-neutral-400 mb-2">
                                    {API_KEY_TO_BASE_URL[key.envVar].description}
                                  </p>
                                  <input
                                    type="text"
                                    id={API_KEY_TO_BASE_URL[key.envVar].envVar}
                                    value={baseUrlValues[API_KEY_TO_BASE_URL[key.envVar].envVar] || ""}
                                    onChange={(e) =>
                                      setBaseUrlValues((prev) => ({
                                        ...prev,
                                        [API_KEY_TO_BASE_URL[key.envVar].envVar]: e.target.value,
                                      }))
                                    }
                                    ...
                                    placeholder={API_KEY_TO_BASE_URL[key.envVar].placeholder}
                                  />
                                  {originalBaseUrlValues[API_KEY_TO_BASE_URL[key.envVar].envVar] && (
                                    <div className="flex items-center gap-1 mt-1">
                                      <svg
                                        className="w-3 h-3 text-green-500 dark:text-green-400"
                                        fill="none"
                                  ...
                                  {/* Bypass proxy switch - only for Anthropic and only when base URL is set */}
                                  {key.envVar === "ANTHROPIC_API_KEY" && baseUrlValues.ANTHROPIC_BASE_URL?.trim() && (
                                    <div className="mt-3 flex items-start justify-between gap-4">
                                      <div>
                                        ...
                                        <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                          When enabled, Claude Code connects directly to your custom URL instead of routing through cmux.
                                        </p>
                                      </div>
                                      <Switch
                                        aria-label="Bypass cmux proxy"
                                        size="sm"
                                        color="primary"
