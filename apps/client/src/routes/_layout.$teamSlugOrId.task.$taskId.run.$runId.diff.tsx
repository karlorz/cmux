...
import { Command, Check, Circle, AlertCircle, Loader2, ChevronRight, ChevronDown, ExternalLink, X } from "lucide-react";
import {
  Suspense,
  memo,
  useCallback,
...
function RunDiffPage() {
  ...
  // Check if taskrun has corresponding PR
  const correspondingPR = useMemo(() => {
    return selectedRun?.pullRequests?.[0];
  }, [selectedRun]);

  // Fetch PR details if there's a corresponding PR
  const prDetails = useQuery(
    api.github_prs.getPullRequest,
    correspondingPR && correspondingPR.number ? {
      teamSlugOrId,
      repoFullName: correspondingPR.repoFullName,
      number: correspondingPR.number,
    } : "skip"
  );

  // Fetch workflow data for the corresponding PR
  ...
  const workflowData = useMemo(() => {
    ...
    const allRuns = [
      ...(workflowRuns || []).map((run: unknown) => ({ ...run, type: 'workflow' as const, name: (run as any).workflowName, timestamp: (run as any).runStartedAt, url: (run as any).htmlUrl })),
      ...(checkRuns || []).map((run: unknown) => {
        const url = (run as any).htmlUrl || `https://github.com/${prDetails.repoFullName}/pull/${prDetails.number}/checks?check_run_id=${(run as any).checkRunId}`;
        return { ...run, type: 'check' as const, timestamp: (run as any).startedAt, url };
      }),
      ...(deployments || []).filter((dep: unknown) => (dep as any).environment !== 'Preview').map((dep: unknown) => ({
        ...dep,
        type: 'deployment' as const,
        name: (dep as any).description || (dep as any).environment || 'Deployment',
        timestamp: (dep as any).createdAt,
        status: (dep as any).state === 'pending' || (dep as any).state === 'queued' || (dep as any).state === 'in_progress' ? 'in_progress' : 'completed',
        conclusion: (dep as any).state === 'success' ? 'success' : (dep as any).state === 'failure' || (dep as any).state === 'error' ? 'failure' : undefined,
        url: (dep as any).targetUrl
      })),
      ...(commitStatuses || []).map((status: unknown) => ({
        ...status,
        type: 'status' as const,
        name: (status as any).context,
        timestamp: (status as any).updatedAt,
        status: (status as any).state === 'pending' ? 'in_progress' : 'completed',
        conclusion: (status as any).state === 'success' ? 'success' : (status as any).state === 'failure' || (status as any).state === 'error' ? 'failure' : undefined,
        url: (status as any).targetUrl
      })),
    ];
