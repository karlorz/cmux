...
export const localWorkspaceConfigsRouter = new OpenAPIHono();

const LocalWorkspaceConfigResponse = z
  .object({
  ...
  .openapi("LocalWorkspaceConfigResponse");

const LocalWorkspaceConfigQuery = z
  .object({
    teamSlugOrId: z.string(),
    projectFullName: z.string(),
  })
  .openapi("LocalWorkspaceConfigQuery");

const LocalWorkspaceConfigBody = z
  .object({
    teamSlugOrId: z.string(),
    projectFullName: z.string(),
    maintenanceScript: z.string().optional(),
  ...
  .openapi("LocalWorkspaceConfigBody");
...
localWorkspaceConfigsRouter.openapi(
  createRoute({
    method: "get",
    path: "/local-workspace-configs",
    summary: "Get local workspace configuration",
    tags: ["LocalWorkspaceConfigs"],
    request: {
      query: LocalWorkspaceConfigQuery,
    },
    responses: {
      200: {
        ...
        content: {
          "application/json": {
            schema: LocalWorkspaceConfigResponse.nullable(),
          },
        },
      },
  ...
  async (c) => {
    ...
    const config = await convex.query(api.localWorkspaceConfigs.get, {
      teamSlugOrId: query.teamSlugOrId,
...
localWorkspaceConfigsRouter.openapi(
  createRoute({
    method: "post",
    path: "/local-workspace-configs",
    summary: "Create or update local workspace configuration",
    tags: ["LocalWorkspaceConfigs"],
    request: {
      body: {
        content: {
          "application/json": {
            schema: LocalWorkspaceConfigBody,
          },
        },
      },
    },
    responses: {
      200: {
        ...
        content: {
          "application/json": {
            schema: LocalWorkspaceConfigResponse,
          },
        },
      },
  ...
  async (c) => {
    ...
    const convex = getConvex({ accessToken });
    const existing = await convex.query(api.localWorkspaceConfigs.get, {
      teamSlugOrId: body.teamSlugOrId,
    ...
    if (!dataVaultKey) {
      dataVaultKey = `local_${randomBytes(16).toString("hex")}`;
    }
    ...
    await convex.mutation(api.localWorkspaceConfigs.upsert, {
      teamSlugOrId: body.teamSlugOrId,
      projectFullName: body.projectFullName,
