...
export const githubPrsOpenCreateRouter = new OpenAPIHono();

githubPrsOpenCreateRouter.openapi(
  createRoute({
  ...
  async (c) => {
    ...
    const existingByRepo = new Map(
      (run.pullRequests ?? []).map((record) => [record.repoFullName, record] as const),
    );
