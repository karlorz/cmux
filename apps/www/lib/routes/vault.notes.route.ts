...
const VaultNotesQuery = z.object({
...
export const vaultNotesRouter = new OpenAPIHono();

vaultNotesRouter.openapi(
  createRoute({
    method: "get" as const,
    path: "/vault/notes",
    tags: ["Vault"],
  ...
  async (c) => {
    ...
    try {
      ...
      const limitedNotes = notes.slice(0, limit).map((note) => ({
        ...
        status: note.status,
        todoCount: note.todos.filter((todo) => !todo.completed).length,
