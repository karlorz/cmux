...
function checkoutPullRequest(workspacePath: string, pullRequestUrl: string) {
  log(`Checking out pull request via gh: ${pullRequestUrl}`);
  const escapedUrl = escapeForDoubleQuotes(pullRequestUrl);
  const { exitCode, stderr } = exec(
    `gh pr checkout "${escapedUrl}"`,
    { cwd: workspacePath, throwOnError: false }
  );
  ...
  if (exitCode !== 0) {
    ...
    throw new Error(
      `gh pr checkout failed: ${stderr || "Unknown error"}`
    );
  }

  log("Pull request checkout completed");
}
