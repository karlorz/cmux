  ...
  async (c) => {
    ...
    try {
      ...
      if (selectedRepos && selectedRepos.length > 0) {
        ...
        if (isSingleRepo) {
          // Single-repo path: clone directly to /root/workspace
          const singleRepo = selectedRepos[0]!;

          // Remove any leftover subdirectory repos (from previous multi-repo layout)
          const listSubdirReposCmd = await Sentry.startSpan(
            { name: "instance.exec (list subdir repos)", op: "sandbox.exec" },
            () =>
              sandboxInstance.exec(
          ...
          const subdirRepos = listSubdirReposCmd.stdout
            .split("\n")
            .filter(Boolean);
          for (const subdirRepo of subdirRepos) {
            console.log(
              `Removing subdirectory repository: ${subdirRepo} (transitioning to single-repo layout)`
            );
            await Sentry.startSpan(
              { name: `instance.exec (rm ${subdirRepo})`, op: "sandbox.exec" },
              () => sandboxInstance.exec(`rm -rf /root/workspace/${subdirRepo}`)
            );
            removedRepos.push(subdirRepo);
          }

          // Check if root repo already matches the selected repo
          const rootRepoMatches =
            hasRootRepo &&
            rootRepoRemote !== "no-remote" &&
            rootRepoRemote.includes(singleRepo);

          if (rootRepoMatches) {
            console.log(
              `Repository ${singleRepo} already exists at workspace root, skipping clone`
            );
          } else {
            // Clear workspace if transitioning from different repo or has stale root repo
            if (hasRootRepo) {
              console.log(
                `Clearing workspace root (different repo or no remote)`
              );
              await Sentry.startSpan(
                { name: "instance.exec (clear root)", op: "sandbox.exec" },
                () =>
                  sandboxInstance.exec(
            ...
            // Clone directly to /root/workspace
            console.log(`Cloning repository ${singleRepo} to workspace root`);
            ...
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
              const cloneCmd = await Sentry.startSpan(
                {
                  name: `instance.exec (clone ${singleRepo} to root)`,
                  op: "sandbox.exec",
                  attributes: { attempt },
                },
                () =>
                  sandboxInstance.exec(
                    `cd /root/workspace && git clone https://github.com/${singleRepo}.git . 2>&1`
                  )
              );

              if (cloneCmd.exit_code === 0) {
                cloneSuccess = true;
                clonedRepos.push(singleRepo);
                break;
              } else {
                ...
                if (isAuthError) {
                  console.error(
                    `Authentication failed for ${singleRepo}: ${lastError}`
                  );
                  break;
                }
                ...
                if (attempt < maxRetries) {
                  console.log(
                    `Clone attempt ${attempt} failed for ${singleRepo}, retrying...`
                  );
            ...
            if (!cloneSuccess) {
              ...
              console.error(
                `Failed to clone ${singleRepo}: ${errorMsg}\nDetails: ${lastError}`
              );
              failedClones.push({
                repo: singleRepo,
          ...
          // Multi-repo path: clone to /root/workspace/{repoName} subdirectories

          // Clear root .git if transitioning from single-repo layout
          if (hasRootRepo) {
            console.log(
              `Clearing workspace root .git (transitioning to multi-repo layout)`
            );
            await Sentry.startSpan(
              { name: "instance.exec (clear root git)", op: "sandbox.exec" },
              () =>
                sandboxInstance.exec(
                  "rm -rf /root/workspace/.git /root/workspace/* /root/workspace/.[!.]* 2>/dev/null || true"
                )
          ...
          for (const [existingName, existingUrl] of existingRepos) {
            ...
            if (!selectedRepo) {
              ...
              await Sentry.startSpan(
                {
                  name: `instance.exec (rm ${existingName})`,
                  op: "sandbox.exec",
                },
                () =>
