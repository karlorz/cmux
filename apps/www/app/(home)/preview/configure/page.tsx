...
import { PreviewConfigForm } from "./_components/preview-config-form";

export const metadata: Metadata = {
  title: "Configure cmux preview — setup scripts and environment",
  description: "Configure dev scripts, environment variables, and browser settings for automatic PR screenshots.",
};
...
export default async function PreviewConfigurePage({
  ...
  if (!user) {
    redirect("/sign-in?after_auth_return_to=/preview/configure");
  }
  ...
  // Parse selected repos from search params
  const selectedRepos = Array.isArray(searchParams.repo)
    ? searchParams.repo
    : searchParams.repo
      ? [searchParams.repo]
      : [];
