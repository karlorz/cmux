import type { PreviewScreenshotUploadPayload } from "@cmux/shared";
import type { Id } from "@cmux/convex/dataModel";
import { promises as fs } from "node:fs";
import * as path from "node:path";

import { log } from "../logger";
import { startScreenshotCollection } from "./startScreenshotCollection";
import { convexRequest } from "../crown/convex";

export interface RunPreviewScreenshotsOptions {
  previewRunId: Id<"previewRuns">;
  token: string;
  convexUrl?: string;
  anthropicApiKey?: string | null;
}

function resolveContentType(filePath: string): string {
  const extension = path.extname(filePath).toLowerCase();
  if (extension === ".jpg" || extension === ".jpeg") {
    return "image/jpeg";
  }
  if (extension === ".webp") {
    return "image/webp";
  }
  return "image/png";
}

async function createScreenshotUploadUrl(
  token: string,
  contentType: string,
  baseUrlOverride?: string,
): Promise<string> {
  const response = await convexRequest<{ ok: true; uploadUrl: string }>(
    "/api/screenshots/preview/upload-url",
    token,
    { contentType },
    baseUrlOverride,
  );

  if (!response?.ok || !response.uploadUrl) {
    throw new Error("Failed to create screenshot upload URL");
  }

  return response.uploadUrl;
}

async function uploadScreenshotFile(params: {
  screenshotPath: string;
  fileName?: string;
  commitSha: string;
  token: string;
  convexUrl?: string;
}): Promise<NonNullable<PreviewScreenshotUploadPayload["images"]>[number]> {
  const { screenshotPath, fileName, commitSha, token, convexUrl } = params;
  const resolvedFileName = fileName ?? path.basename(screenshotPath);
  const contentType = resolveContentType(screenshotPath);

  const uploadUrl = await createScreenshotUploadUrl(
    token,
    contentType,
    convexUrl,
  );

  const bytes = await fs.readFile(screenshotPath);
  const uploadResponse = await fetch(uploadUrl, {
    method: "POST",
    headers: {
      "Content-Type": contentType,
    },
    body: new Uint8Array(bytes),
  });

  if (!uploadResponse.ok) {
    const body = await uploadResponse.text();
    throw new Error(
      `Upload failed with status ${uploadResponse.status}: ${body}`,
    );
  }

  const uploadResult = (await uploadResponse.json()) as {
    storageId?: string;
  };
  if (!uploadResult.storageId) {
    throw new Error("Upload response missing storageId");
  }

  return {
    storageId: uploadResult.storageId,
    mimeType: contentType,
    fileName: resolvedFileName,
    commitSha,
  };
}

export async function runPreviewScreenshots(
  options: RunPreviewScreenshotsOptions,
): Promise<void> {
  const { previewRunId, token, convexUrl, anthropicApiKey } = options;

  log("INFO", "Starting preview screenshot workflow", {
    previewRunId,
    hasAnthropicKey: Boolean(anthropicApiKey ?? process.env.ANTHROPIC_API_KEY),
  });

  const result = await startScreenshotCollection({
    anthropicApiKey: anthropicApiKey ?? undefined,
    taskRunJwt: token,
  });

  let images: PreviewScreenshotUploadPayload["images"];
  let status: PreviewScreenshotUploadPayload["status"] = "failed";
  let error: string | undefined;

  if (result.status === "completed") {
    const capturedScreens = result.screenshots ?? [];
    if (capturedScreens.length === 0) {
      status = "failed";
      error = "Claude collector returned no screenshots";
      log("ERROR", error, { previewRunId });
    } else {
      const uploadPromises = capturedScreens.map((screenshot) =>
        uploadScreenshotFile({
          screenshotPath: screenshot.path,
          fileName: screenshot.fileName,
          commitSha: result.commitSha,
          token,
          convexUrl,
        }),
      );

      const settledUploads = await Promise.allSettled(uploadPromises);
      const successfulScreens: NonNullable<
        PreviewScreenshotUploadPayload["images"]
      > = [];
      const failures: { index: number; reason: string }[] = [];

      settledUploads.forEach((settled, index) => {
        if (settled.status === "fulfilled") {
          successfulScreens.push(settled.value);
        } else {
          const reason =
            settled.reason instanceof Error
              ? settled.reason.message
              : String(settled.reason);
          failures.push({ index, reason });
          log("ERROR", "Failed to upload preview screenshot", {
            previewRunId,
            screenshotPath: capturedScreens[index]?.path,
            error: reason,
          });
        }
      });

      if (failures.length === 0) {
        images = successfulScreens;
        status = "completed";
        log("INFO", "Preview screenshots uploaded", {
          previewRunId,
          screenshotCount: successfulScreens.length,
          commitSha: result.commitSha,
        });
      } else {
        status = "failed";
        error =
          failures.length === 1
            ? failures[0]?.reason
            : `Failed to upload ${failures.length} screenshots`;
      }
    }
  } else if (result.status === "skipped") {
    status = "skipped";
    error = result.reason;
    log("INFO", "Preview screenshot workflow skipped", {
      previewRunId,
      reason: result.reason,
    });
  } else if (result.status === "failed") {
    status = "failed";
    error = result.error;
    log("ERROR", "Preview screenshot workflow failed", {
      previewRunId,
      error: result.error,
    });
  } else {
    status = "failed";
    error = "Unknown screenshot workflow result";
    log("ERROR", "Preview screenshot workflow returned unknown status", {
      previewRunId,
      result,
    });
  }

  // Get commitSha from result (or use empty string as fallback)
  const commitSha =
    result.status === "completed" ? result.commitSha : "unknown";

  // Upload screenshot metadata
  await convexRequest(
    "/api/screenshots/preview/upload",
    token,
    {
      previewRunId,
      status,
      images,
      error,
      commitSha,
    },
    convexUrl,
  );

  log("INFO", "Preview screenshot metadata uploaded", {
    previewRunId,
    status,
  });
}
