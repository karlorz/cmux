---
name: devsh
description: Development build of devsh CLI. Same features as devsh (npm) but built from local source for development/testing. Use devsh when developing the CLI itself.
---

# devsh - Development Build of devsh CLI

> **Note**: `devsh` is built from local source (`packages/devsh`) and used when developing or testing the CLI itself. For production use, install `devsh` via npm.

| Binary | Purpose | Installation |
|--------|---------|--------------|
| `devsh` | Production CLI | `npm install -g devsh` |
| `devsh` | Development CLI | `make install-devsh-dev` |

Both builds have identical commands - only the build source differs.
