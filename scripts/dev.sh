...
cleanup() {
    ...
    # Ignore signals during cleanup to ensure it completes
    trap '' INT TERM HUP QUIT PIPE
    trap - EXIT
    # Prevent early exits on command failures during cleanup
    set +e
...
if [ "$SKIP_CONVEX" = "true" ]; then
    ...
    if [ "$IS_DEVCONTAINER" = "true" ]; then
        ...
        (cd .devcontainer && exec bash -c 'trap "pkill -9 -P $$ 2>/dev/null || true" EXIT; \
          COMPOSE_PROJECT_NAME=cmux-convex docker compose -f docker-compose.convex.yml up </dev/null 2>&1 | tee "$LOG_DIR/docker-compose.log" | { \
