...
URL PATTERN (Morph-consistent, uses free Cloudflare Universal SSL):
    https://port-{port}-vm-{vmid}.{CF_DOMAIN}

    Examples:
      https://port-39378-vm-200.example.com  (vscode)
      https://port-39377-vm-200.example.com  (worker)
      https://port-39375-vm-200.example.com  (exec)
      https://port-39380-vm-200.example.com  (vnc)
      https://port-39383-vm-200.example.com  (xterm)
      https://port-5173-vm-200.example.com   (preview)
...
# Generated by pve-tunnel-setup.sh
# URL pattern (Morph-consistent): port-{port}-vm-{vmid}.${CF_DOMAIN}
#
# Examples:
#   port-39378-vm-200.${CF_DOMAIN} -> cmux-200.${domain_suffix}:39378 (vscode)
#   port-39377-vm-200.${CF_DOMAIN} -> cmux-200.${domain_suffix}:39377 (worker)
#   port-39375-vm-200.${CF_DOMAIN} -> cmux-200.${domain_suffix}:39375 (exec)
#   port-39380-vm-200.${CF_DOMAIN} -> cmux-200.${domain_suffix}:39380 (vnc)
#   port-39383-vm-200.${CF_DOMAIN} -> cmux-200.${domain_suffix}:39383 (xterm)
#   port-5173-vm-200.${CF_DOMAIN}  -> cmux-200.${domain_suffix}:5173  (preview)

:${CADDY_PORT} {
    # Generic port-based routing: port-{port}-vm-{vmid}.{domain}
    # Single rule handles all services - no hardcoded service names
    ...
    handle {
        respond "cmux sandbox not found. Use format: port-{port}-vm-{vmid}.${CF_DOMAIN}" 404
    }
...
show_status() {
    ...
    if [[ -n "${CF_DOMAIN:-}" ]]; then
        echo "Sandbox URLs: https://port-{port}-vm-{vmid}.${CF_DOMAIN}"
    fi
}
...
full_setup() {
    ...
    echo ""
    echo "URLs will be (Morph-consistent pattern, free Cloudflare Universal SSL):"
    echo "  https://port-39378-vm-{vmid}.${CF_DOMAIN}  (vscode)"
    echo "  https://port-39377-vm-{vmid}.${CF_DOMAIN}  (worker)"
    echo ""
    ...
    echo "  https://port-39378-vm-{vmid}.${CF_DOMAIN}  (vscode)"
    echo "  https://port-39377-vm-{vmid}.${CF_DOMAIN}  (worker)"
    echo "  https://port-39375-vm-{vmid}.${CF_DOMAIN}  (exec)"
    echo "  https://port-39380-vm-{vmid}.${CF_DOMAIN}  (vnc)"
    echo "  https://port-39383-vm-{vmid}.${CF_DOMAIN}  (xterm)"
    echo ""
    echo "To test, start a sandbox container and visit the URL."
    echo "For example, if you start cmux-200:"
    echo "  https://port-39378-vm-200.${CF_DOMAIN}"
}
