...
WAIT_OUTPUT=$(timeout 10s cmux-devbox orchestrate wait invalid_id_12345 --timeout 5s 2>&1 || true)
...
WAIT_TIMEOUT_OUTPUT=$(timeout 10s cmux-devbox orchestrate wait fake_task_that_does_not_exist --timeout 3s 2>&1 || true)
...
MSG_NO_TYPE=$(cmux-devbox orchestrate message fake_task_run_id "test message" 2>&1 || true)
...
MSG_INVALID_TYPE=$(cmux-devbox orchestrate message fake_task_run_id "test message" --type invalid_type 2>&1 || true)
...
MIGRATE_NO_FILE=$(cmux-devbox orchestrate migrate 2>&1 || true)
