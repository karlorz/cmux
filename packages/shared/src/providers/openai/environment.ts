 ...
 * This regex matches from [notice.model_migrations] to the next section header or EOF
 */
...
`;
  ...
  // Also ensure model_migrations are set to suppress deprecation notices
  try {
    ...
    // Strip existing model_migrations and append our managed ones
    ...
    // No host config.toml; create a minimal one with notify and model_migrations
    const toml =
      `notify = ["/root/lifecycle/codex-notify.sh"]\n` +
