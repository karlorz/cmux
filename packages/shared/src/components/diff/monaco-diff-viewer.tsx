...
import { cn } from "./cn";
import { FileDiffHeader, type FileDiffHeaderProps } from "./file-diff-header";
...
export interface FileEditorReadyEvent {
...
export interface MonacoDiffViewerProps extends GitDiffViewerProps {
  monacoTheme: string;
  loader: Promise<MonacoInstance>;
  enableDebugLogging?: boolean;
  getKitty?: () => string;
  FileDiffHeaderComponent?: (props: FileDiffHeaderProps) => ReactNode;
  onFileEditorReady?: (event: FileEditorReadyEvent) => void;
  getRowId?: (file: MonacoFileGroup) => string | undefined;
}
  ...
  const onEditorMount = useMemo(
    () =>
      createDiffEditorMount({
        ...
        onReady: ({ controls, diffEditor, container, applyLayout, monaco }) => {
    ...
    [editorMinHeight, file, onFileEditorReady],
  );
...
export { MonacoDiffViewer as MonacoGitDiffViewer };
...
const MemoMonacoFileDiffRow = memo(MonacoFileDiffRow, (prev, next) => {
  ...
  return (
    ...
    prev.FileDiffHeaderComponent === next.FileDiffHeaderComponent &&
    prev.onFileEditorReady === next.onFileEditorReady &&
...
export function MonacoDiffViewer({
  ...
  const kittySource = getKitty ?? getRandomKitty;
  const kitty = useMemo(() => kittySource(), [kittySource]);
  ...
  const shouldLog = useMemo(() => {
    if (typeof enableDebugLogging === "boolean") {
    ...
    return !isProduction || isElectronEnvironment;
  }, [enableDebugLogging]);

  const expandAll = () => {
    debugGitDiffViewerLog(shouldLog, "expandAll invoked", {
  ...
  const collapseAll = () => {
    debugGitDiffViewerLog(shouldLog, "collapseAll invoked", {
      fileCount: fileGroups.length,
    });
  ...
  const setFileExpanded = (filePath: string, expanded: boolean) => {
    setExpandedFiles((prev) => {
      const alreadyExpanded = prev.has(filePath);
      ...
      try {
        onFileToggle?.(filePath, expanded);
      } catch {
        // ignore
      }
      ...
      <div className="flex flex-col -space-y-[2px]">
        {fileGroups.map((file, index) => (
          <MemoMonacoFileDiffRow
            ...
            FileDiffHeaderComponent={FileDiffHeaderComponent}
            onFileEditorReady={onFileEditorReady}
            rowId={getRowId?.(file)}
          />
        ))}
