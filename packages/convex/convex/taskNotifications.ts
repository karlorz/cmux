...
export const getUnreadCount = authQuery({
  ...
  handler: async (ctx, args) => {
    ...
    // Count unread runs for this user in this team (capped for safety)
...
export const getTasksWithUnread = authQuery({
  ...
  handler: async (ctx, args) => {
    ...
    // Get all unread runs for this user in this team (capped for safety)
    const unreadRuns = await ctx.db
      .query("unreadTaskRuns")
      .withIndex("by_team_user", (q) =>
        q.eq("teamId", teamId).eq("userId", userId),
...
export const markTaskAsUnread = authMutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Filter runs belonging to this team
    const teamRuns = runs.filter((run) => run.teamId === teamId);
...
export const markAllAsRead = authMutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Delete all unread rows in parallel (O(N) parallel instead of O(N) sequential)
    await Promise.all(unreadRuns.map((unread) => ctx.db.delete(unread._id)));
  },
});
