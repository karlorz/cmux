...
function rewriteMorphUrl(url: string): string {
  ...
  if (url.includes("http.cloud.morph.so")) {
    try {
      ...
      if (hostnameParts.length >= 3 && hostnameParts[0] === "port") {
        const port = hostnameParts[1];
        const morphId = hostnameParts.slice(2).join("-").replace("morphvm-", "");
        urlObj.hostname = `cmux-${morphId}-base-${port}.cmux.app`;
        return urlObj.toString();
      }
    } catch {
      // Fall back to old logic if URL parsing fails
    }
    const result = url
      .replace("morphvm-", "")
