...
const STOP_DAYS_THRESHOLD = 7; // 1 week
    ...
    for (const config of configs) {
      ...
      try {
        ...
        for (const orphan of orphans) {
          ...
          // For PVE-LXC without creation time, add extra safety: check devboxInfo table
          if (orphan.created === 0) {
            if (config.provider === "pve-lxc") {
              // Extra safety: check if instance exists in devboxInfo table
              ...
              // No devbox record + no activity record + stopped = truly orphaned
              console.log(
