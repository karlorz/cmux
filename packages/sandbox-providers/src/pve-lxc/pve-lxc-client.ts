...
import type { ExecResult, HttpService } from "../types";
...
/**
 * Configuration options for PveLxcClient
 */
export interface PveLxcClientOptions {
  apiUrl: string;
  apiToken: string;
  node?: string;
  publicDomain?: string;
  verifyTls?: boolean;
  /**
   * Function to resolve snapshot ID to template VMID.
   * If not provided, uses dynamic import from @cmux/shared.
   */
  snapshotResolver?: (snapshotId: string) => Promise<{ templateVmid: number }>;
}
...
export class PveLxcClient {
  ...
  /** HTTPS agent for PVE API calls */
  private httpsAgent: Agent;
  /** Custom snapshot resolver function */
  private snapshotResolver?: (snapshotId: string) => Promise<{ templateVmid: number }>;
  ...
  constructor(options: PveLxcClientOptions) {
    ...
    this.httpsAgent = new Agent({
      connect: {
        rejectUnauthorized: Boolean(options.verifyTls),
      },
    });
  }
  ...
  private async parseSnapshotId(snapshotId: string): Promise<{
    ...
    const { PVE_LXC_SNAPSHOT_PRESETS } = await import("@cmux/shared");
...
/**
 * Create a PVE LXC client instance from options
 */
export function createPveLxcClient(options: PveLxcClientOptions): PveLxcClient {
  return new PveLxcClient(options);
}
