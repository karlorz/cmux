...
import { PVE_LXC_SNAPSHOT_PRESETS } from "@cmux/shared/pve-lxc-snapshots";
...
/**
 * Configuration for creating a PVE LXC client.
 */
...
export class PveLxcClient {
  ...
  private pveHttpsAgent: Agent;

  constructor(options: PveLxcClientConfig) {
    this.apiUrl = options.apiUrl.replace(/\/$/, "");
    this.apiToken = options.apiToken;
    this.node = options.node || null; // Will be auto-detected if not provided
    this.publicDomain = options.publicDomain || null;
    // PVE deployments often use self-signed certs; default is no TLS verification.
    this.pveHttpsAgent = new Agent({
      connect: {
