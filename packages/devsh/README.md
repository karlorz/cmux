...
./bin/devsh exec <id> "echo '$B64' | base64 -d > /usr/loca./bin/devsh-worker && chmod +x /usr/loca./bin/devsh-worker"

# 3. Create token directory and start worker
./bin/devsh exec <id> "mkdir -p /var/run/cmux"
./bin/devsh exec <id> "nohup node /usr/loca./bin/devsh-worker > /var/log/devsh-worker.log 2>&1 &"
...
2. Embeds the devsh-worker script at `/usr/loca./bin/devsh-worker`
