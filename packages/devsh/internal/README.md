...
This document is for maintainers working on `packages/devsh`.
...
## Install (make `devsh` available on PATH)

Pick one of the options below so you can run `devsh --help` directly.
...
```bash
cd packages/devsh
GOBIN="$HOME/.local/bin" go install ./cmd/devsh
