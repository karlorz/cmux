...
/**
 * Resume the Morph VM for a task run if it is paused
 */
export const postApiTaskRunsByIdForceWakeMutation = (options?: Partial<Options<PostApiTaskRunsByIdForceWakeData>>): UseMutationOptions<PostApiTaskRunsByIdForceWakeResponse, DefaultError, Options<PostApiTaskRunsByIdForceWakeData>> => {
    const mutationOptions: UseMutationOptions<PostApiTaskRunsByIdForceWakeResponse, DefaultError, Options<PostApiTaskRunsByIdForceWakeData>> = {
        mutationFn: async (fnOptions) => {
            const { data } = await postApiTaskRunsByIdForceWake({
                ...options,
                ...fnOptions,
                throwOnError: true
            });
