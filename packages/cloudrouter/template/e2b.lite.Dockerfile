# E2B Dockerfile for cmux-lite template
# A lighter devbox without Docker-in-Docker or JupyterLab
# Optimized for lower resource usage while maintaining core functionality
...
# Install system packages (minimal set for devbox)
RUN apt-get update && apt-get install -y \
    ...
    openssl \
    rsync \
    # VNC and desktop environment (lightweight)
    xfce4 \
    xfce4-goodies \
    dbus-x11 \
...
COPY worker/start-services-lite.sh /usr/local/bin/start-services.sh
...
# Make sure user can run services
...
# Expose ports
# 39377 - Worker daemon (HTTP API)
# 39378 - VSCode (cmux-code)
# 39380 - VNC (noVNC)
# 9222 - Chrome CDP (localhost only, proxied via worker)
EXPOSE 39377 39378 39380
