...
// buildSSHProxyCommand creates an SSH command for use with ProxyCommand tunneling.
// Uses sshpass when available, otherwise creates a wrapper script that self-hosts SSH_ASKPASS.
func buildSSHProxyCommand(proxyCmd string) (string, string, error) {
	baseArgs := fmt.Sprintf("-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o PubkeyAuthentication=no -o ProxyCommand=%q", proxyCmd)

	if _, err := exec.LookPath("sshpass"); err == nil {
		return fmt.Sprintf("sshpass -p '' ssh %s", baseArgs), "", nil
	}
...
func runRsyncSingleFileWithCurl(curlPath, wsURL, token, localFile, remotePath string) (*rsyncStats, error) {
	...
	sshCmd, scriptPath, err := buildSSHProxyCommand(proxyCmd)
	if err != nil {
		return nil, err
	}
	if scriptPath != "" {
